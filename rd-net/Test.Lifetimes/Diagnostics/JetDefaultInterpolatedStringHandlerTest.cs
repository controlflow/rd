...
public class JetDefaultInterpolatedStringHandlerTest
{
  [Test]
  [TestCase(0, 0)]
  [TestCase(1, 1)]
  ...
  public void LengthAndHoleArguments_Valid(int literalLength, int formattedCount)
  {
    ...
    foreach (var provider in new IFormatProvider[]
             {
               null, new ConcatFormatter(), CultureInfo.InvariantCulture, CultureInfo.CurrentCulture,
  ...
  {
    var handler = new JetDefaultInterpolatedStringHandler(0, 0);
  ...
  public void AppendLiteral()
  {
    ...
    var actual = new JetDefaultInterpolatedStringHandler(0, 0);
  ...
  public void AppendFormatted_ReadOnlySpanChar()
  {
    ...
    foreach (var s in new[] { "", "a", "bc", "def", "this is a longer string", "!" })
    {
      // span
      expected.Append(s);
      ...
      foreach (var alignment in new[] { 0, 3, -3 })
      {
        // span, alignment
  ...
  public void AppendFormatted_String()
  {
    ...
    foreach (var s in new[] { null, "", "a", "bc", "def", "this is a longer string", "!" })
    {
      // string
  ...
  public void AppendFormatted_String_ICustomFormatter()
  {
    ...
    var expected = new StringBuilder();
    var actual = new JetDefaultInterpolatedStringHandler(0, 0, provider);

    foreach (var s in new[] { null, "", "a" })
    {
      // string
  ...
  public void AppendFormatted_ReferenceTypes()
  {
    ...
    foreach (var rawInput in new[] { null, "", "a", "bc", "def", "this is a longer string", "!" })
    {
  ...
  public void AppendFormatted_ReferenceTypes_CreateProviderFlowed(bool useScratch)
  {
    ...
    foreach (var tss in new IHasToStringState[] { new FormattableStringWrapper("hello") })
    {
  ...
  public void AppendFormatted_ReferenceTypes_ICustomFormatter()
  {
    ...
    foreach (var s in new[] { null, "", "a" })
    {
      foreach (var tss in new IHasToStringState[] { new FormattableStringWrapper(s) })
      {
  ...
  {
    var expected = tss is IFormattable ? ToStringMode.IFormattableToString : ToStringMode.ObjectToString;
  ...
  private sealed class ConcatFormatter : IFormatProvider, ICustomFormatter
  {
    ...
    {
      var s = format + " " + arg + formatProvider;
